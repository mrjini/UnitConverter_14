...
| **O — Output (산출물)** | 단계별 결과물 | **SPEC:** PRD, R-G-I-O, ECB 설계(08), Harness(`src/unit_converter/{entity,control,boundary,infrastructure}/`), 시나리오·추적 매트릭스. **RED:** pytest. **GREEN:** ECB 레이어별 구현. **REFACTOR:** boundary→control→entity 정렬. **new_features:** infrastructure·boundary 확장. |
